...
\pdfinfo{
/Title (Finding the Balance: An Exploration of Metrics to Design Balanced Character Stats in Auto Battler Games)
...
\title{Finding the Balance: An Exploration of Metrics to Design Balanced Character Stats in Auto Battler Games}%{My Publication Title --- Multiple Authors}
\author {
    % Authors
...
\affiliations {
    ...
    budijono@affiliation1.com, goldman@affilation2.com, maloney@affiliation1.com, jmueller@sift.net, pwalker@sift.net, jladwig@sift.net, rfreedman@sift.net
}
%\fi
...
\begin{abstract}
The auto battler is a new video game genre that has been growing in popularity.
Although the majority of the gameplay is automated via scripted rules, players
select characters from a pool of options and arrange them on the game board
before the automation begins.  Like other game formats with many unique components
that players assemble to customize their content, such as
trading card games, designers must be careful to balance the components so that
players feel that there are many reasonable choices of component combinations.
We developed a framework that combines automated playtesting with global search
to identify stats for each component that will assist designers in achieving this
balance.  To guide the global search, we define metrics describing different
elements of balancing and explore their impacts on the designed game components.
This work introduces our auto battler game and platform designed for research
use, describes the framework for balancing the game components used in customized
content, and provides a preliminary investigation into our exploration of metrics
and their impacts on various design challenges related to balancing for auto battler games.
\end{abstract}
